...
object CrowdSecIntegrationSuite {
   ...
   * below 1.40 with a bare 400 — which surfaces as "could not find a valid Docker environment"
   * even though the socket is perfectly reachable.
   *
   * 1.41 shipped with Docker 20.10 and every daemon since accepts it. Only applied when the
   * machine has not already pinned a version, so an explicit local setting always wins.
   */
  def pinDockerApiVersion(): Unit = {
    val configured =
