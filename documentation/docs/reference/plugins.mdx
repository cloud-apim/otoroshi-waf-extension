...
|---|---|---|
| Cloud APIM Security Suite - Preset | `NgPresetPlugin` | A route |
...
| Cloud APIM Security Suite - Fail2ban | `NgAccessValidator` + `NgRequestTransformer` | A route |
...
5. Cloud APIM WAF               inspect the payload
6. Threat response              read the accumulated score, apply one tier
```

The response plugin must come **after** the WAF, or it reads a score the WAF has not contributed to
yet — silently, with every request still flowing. See [the fabric](../fabric/overview.mdx).

The [preset plugin](../fabric/preset.mdx) expands into that exact chain with explicit
`plugin_index` values, which is the reason to prefer it over composing the five slots by hand.
