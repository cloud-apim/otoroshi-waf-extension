...
Emitted when exclusions are applied from a [learning run](../waf/learning.mdx). It carries the window
the decision was taken on, not only the change.
...
Emitted for every operator action taken on the
[Bans & incidents console](../fabric/bans.mdx) — a ban, an extension, an unban, an allowlist entry,
or an incident moved to a new state.
