...
| `POST` | `/_geo` | `{"ips":["1.2.3.4", …]}` | Network, AS number and registration country of up to 500 addresses, from the enabled ASN databases — what Threat Studio shows next to a caller |
...
on the [allowlist](../fabric/bans.mdx#the-allowlist), and `/_extend` refuses when there is no live
...
[`CloudApimWafSecurityAudit`](./events.mdx#cloudapimwafsecurityaudit) with the operator on it.
