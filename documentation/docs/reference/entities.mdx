...
All five entities are ordinary Otoroshi entities: full CRUD in the admin UI, the admin API,
...
|---|---|---|
| `min_score` | number | — |
| `action` | `log` \| `tarpit` \| `deny` \| `ban` | `log` |
| `tarpit_millis` | number | `3000` |
| `ban_for_seconds` | number | `3600` |
| `status` | number | `403` |

An unrecognised action degrades to `log`, never to `deny`.
