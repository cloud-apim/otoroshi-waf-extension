 ...
 *
 * `setQuery` merges a patch into the current query — `null`, `undefined` and `''` remove a key — and
 * replaces the history entry rather than pushing one (unless `{ push: true }`, for a change of tab),
 * so the back button is not a list of every filter that was tried. It reads `window.location` rather than the rendered query so two patches in
 * the same tick do not overwrite each other.
 */
