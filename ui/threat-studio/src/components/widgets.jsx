...
const viewKey = (view) => `threat-studio.view.${view}`;
 ...
 * filled in from it, so a reload of the page keeps it too. `auto` is always written, `false` included:
 * an absent key would fall back to the remembered value.
 */
export function useTimeView(view, query, setQuery, defaultPeriod) {
  ...
  useEffect(() => {
    const missing = Object.fromEntries(
      ['period', 'auto', 'every'].filter((k) => query[k] === undefined && stored[k] !== undefined).map((k) => [k, stored[k]])
    );
...
/**
 * Reload now, or every so often.
 *
 * Sits next to a `PeriodPicker`: a period that ends `now` moves, and a console left open on an attack
 * should follow it. Controlled, so the page can keep it in its query string (see `useTimeView`). A tick
 * is skipped while the tab is hidden or the previous reload is still running, so a slow query never
 * piles up behind itself.
 */
export function RefreshControl({ onRefresh, busy = false, auto, every, onChange, loadedAt }) {
  ...
  return (
    <div className="refresh-control">
      ...
      <Select
        className="sm"
        value={every}
        onChange={(v) => onChange({ auto, every: v })}
