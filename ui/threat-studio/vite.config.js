...
// `/extensions/cloud-apim/threat-studio` backoffice route (see studio/studio.scala). The build output
// is committed in the extension resources so `sbt assembly` never needs node.
export default defineConfig(({ command, mode }) => {
  const env = loadEnv(mode, process.cwd(), '');
  const otoroshi = env.VITE_OTOROSHI_URL || 'http://otoroshi.oto.tools:9999';
  return {
    plugins: [react()],
    base: command === 'build' ? '/extensions/assets/cloud-apim/extensions/waf/studio/' : '/',
    build: {
      outDir: path.resolve(import.meta.dirname, '../../src/main/resources/cloudapim/extensions/waf/studio'),
      emptyOutDir: true,
    ...
    server: {
      // open http://studio-dev.oto.tools:5174/extensions/cloud-apim/threat-studio: the otoroshi
      // session cookie is set on `.oto.tools` so the proxied api calls are authenticated
      host: '127.0.0.1',
      port: 5174,
      strictPort: true,
